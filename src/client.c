...
void client_join_world(game *gameState) {
    ...
    worldRequestPacket sendPacket;
    sendPacket.type = CONNECT_WORLD;
    strcpy(sendPacket.playerName, gameState->entityState.name);
    strcpy(sendPacket.worldName, gameState->worldName);
            ...
            if (event.type == ENET_EVENT_TYPE_RECEIVE) {
                ...
                switch (messageHeader->type) {
                    ...
                    case BROADCAST_WORLD: {
                        worldConnectPacket *receivedData = (worldConnectPacket*)event.packet->data;
                        enet_peer_disconnect(peer, 0);
                        enet_host_destroy(client);
                        enet_deinitialize();
                        SDL_Delay(500);
                        client_world_init(gameState, receivedData->port);
                        break;
                    }
                }
                enet_packet_destroy(event.packet);
...
void client_player_init(game *gameState) {
    ...
    if (enet_host_service(client, &event, 10000) > 0 && event.type == ENET_EVENT_TYPE_CONNECT) {
        ...
        if (enet_host_service(client, &event, 10000) > 0 &&
            event.type == ENET_EVENT_TYPE_RECEIVE) {
            SDL_Thread *thread = SDL_CreateThread((SDL_ThreadFunction)client_iterate, "client_iterate", gameState);
